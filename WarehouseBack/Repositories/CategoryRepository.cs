...
namespace WarehouseBack.Repositories
{
    internal class CategoryRepository : Repository
    {
        ...
        internal List<Category> GetChildCategories(int? categoryId)
        {
            List<Category> result = new List<Category>();

            command = database.GetStoredProcCommand("spGetChildsCategory");
            if (categoryId.HasValue)
                database.AddInParameter(command, "@ParentID", SqlDbType.Int, categoryId.Value);
